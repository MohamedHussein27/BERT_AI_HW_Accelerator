module systolic_tb;
    ...
    task load_weights( input logic [DATAWIDTH-1:0] W_tile [1:0][1:0]);
    ...
    initial begin
        ...
        for (int cycle = 0; cycle < 5; cycle++) begin
            ...
            for (int row = 0; row < N_SIZE; row++) begin
                matrix_A[row] = A_tile1[row][cycle];
            end
            
            // matrix_B = 0 for first tile (no partial sums)
            for (int col = 0; col < N_SIZE; col++) begin
                matrix_B[col] = 0;
            end
            
            $display("Cycle %0d: A[0]=%2d A[1]=%2d", 
                     cycle, matrix_A[0], matrix_A[1]);
            ...
            if (cycle >= 1 && cycle <= 3) begin
                ...
                $display("  --> Output row %0d captured: [%0d, %0d]", 
                         out_row, partial_sum[out_row][0], partial_sum[out_row][1]);
            end
        end

        // Wait one more clock cycle to capture the final output row
        partial_sum[3][0] = matrix_C[0];
        partial_sum[3][1] = matrix_C[1];
        $display("  --> Output row 3 captured: [%0d, %0d]", 
                 partial_sum[3][0], partial_sum[3][1]);
        ...
        for (int cycle = 0; cycle < 5; cycle++) begin
            // Feed data to each input row from its sequence
            for (int row = 0; row < N_SIZE; row++) begin
                matrix_A[row] = A_tile2[row][cycle];
            end
