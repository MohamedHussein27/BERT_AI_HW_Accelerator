        ...
        begin
            if (!rst_n) 
                ...
                begin
                    ...
                    else if (bram_addr == ((2 * FETCH_START_OFFSET) + transpose_addr_pointer) - ORIGINAL_COLUMNS && FETCH_START_OFFSET == ((ORIGINAL_COLUMNS*ORIGINAL_ROWS*NUM_BITS)/DATA_WIDTH))
                        begin
                ...
                // Increment the pointer AFTER a fetch completes
                    ...
                    /*else if (current_state == DONE && !hold_addr_ptr)
                        begin
                            addr_ptr               <= addr_ptr + 1;
                        end*//*
...
module fetch_logic_gen #(
    ...
    // Internal Address Pointers (Separated for Inputs and Weights)
    reg [9:0] in_addr_ptr;
    reg [9:0] wt_addr_ptr;
    reg [9:0] active_addr_ptr; // Combinational wire to select which pointer to use
        ...
        begin
            if (!rst_n) 
                ...
                begin
                    ...
                    // reset the i (row) and j (column) counter in case the tile is not finished and the rows have reached their max  
                    else if (bram_addr == ((2 * FETCH_START_OFFSET) + transpose_addr_pointer) - ORIGINAL_COLUMNS && FETCH_START_OFFSET == ((ORIGINAL_COLUMNS*ORIGINAL_ROWS*NUM_BITS)/DATA_WIDTH))
                        begin
                            transpose_addr_pointer <= transpose_addr_pointer + 1;
                            i <= 0;
                    ...
                    // ==========================================
                    // INDEPENDENT POINTER RESET & INCREMENT LOGIC
                    // ==========================================
                    
                    // 1. Input Pointer Logic
                    if (reset_in_addr_counter) 
                        ...
                        begin
                            in_addr_ptr            <= in_addr_ptr + 1;
                        end

                    // 2. Weight Pointer Logic
                    if (reset_wt_addr_counter) 
                        begin
                            wt_addr_ptr <= 0;
                            transpose_addr_pointer <= 0;
                        end
                    else if (current_state == FETCHING && next_state == DONE && !hold_addr_ptr && Tiles_Control == 2'b01) // incerement at transition as not to incerement the wrong pointer 
                        begin
                            wt_addr_ptr <= wt_addr_ptr + 1;
                        end
    ...
    always @(*) 
        begin
            ...
            if (Tiles_Control == 2'b01)
                active_addr_ptr = wt_addr_ptr;
            else
                active_addr_ptr = in_addr_ptr;

            // The address is the active pointer value (tile index) multiplied by
            // the number of reads per tile, plus the intra-tile offset.
            if (FETCH_START_OFFSET != ((ORIGINAL_COLUMNS*ORIGINAL_ROWS*NUM_BITS)/DATA_WIDTH))
                begin
                    bram_addr  = (active_addr_ptr * NUM_FETCHES_PER_TILE) + fetch_offset + FETCH_START_OFFSET; 
                end
            else
                begin
                    bram_addr = ORIGINAL_COLUMNS * j + i + transpose_addr_pointer + FETCH_START_OFFSET;
