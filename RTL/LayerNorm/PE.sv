...
module processing_element #(
    ...
    input  logic signed [31:0]     bcast_data,   // Broadcasted scalar (e.g., mean or 1/sigma (standard deviation))
    ...
    // --------------------------------------------------------
    // Local Memory (Registers for Gamma and Beta)
    // --------------------------------------------------------
    logic signed [31:0] local_weight_reg; // Gamma
    logic signed [31:0] local_bias_reg;   // Beta
    logic signed [31:0] local_mean_reg;   // local memory for mean to be used in normalization stage
    ...
    // --------------------------------------------------------
    // Stage 2: The Multiplier (MUL) with Rounding and Clamping
    // --------------------------------------------------------
    logic signed [63:0] mul_full; 
    logic signed [63:0] mul_rounded; // 64-bit to hold the shifted/rounded value
    logic signed [31:0] mul_out;
    
    always_comb begin
        // 1. Perform the full 64-bit multiplication
        case (opcode)
            OP_VAR_SQR:   mul_full = sub_out * sub_out;       
            OP_NORMALIZE: mul_full = sub_out * bcast_data;    
            OP_AFFINE:    mul_full = sub_out * local_weight_reg; 
            default:      mul_full = {32'd0, sub_out};        // Pad bypassed data
        endcase
        
        // 2. Rounding, Shifting, and Clamping
        if (opcode == OP_VAR_SQR || opcode == OP_NORMALIZE || opcode == OP_AFFINE) begin
            // Add half (2^25) for rounding, then shift right by 26
            mul_rounded = (mul_full + (64'(1) << (Q_FRAC_BITS - 1))) >>> Q_FRAC_BITS;
            
            // Saturation Clamping (Protect against losing those 5 integer bits!)
            if (mul_rounded > 64'sh000000007FFFFFFF) begin
                mul_out = 32'sh7FFFFFFF; // Clamp to max positive
            end else if (mul_rounded < -64'sh0000000080000000) begin
                mul_out = -32'sh80000000; // Clamp to max negative
            end else begin
                mul_out = mul_rounded[31:0]; // Safe to take bottom 32 bits
            end
